...
public extension ChatView {
    private static func mapMessages(_ messages: [Message], chatType: ChatType, replyMode: ReplyMode) -> [MessagesSection] {
        ...
        for (i, date) in dates.enumerated() {
            let section = MessagesSection(
                date: date,
                rows: mapMessagesToRows(
                    messages: messages.filter({ $0.createdAt.isSameDay(date) }),
                    chatType: chatType,
                    replyMode: replyMode,
                    isFirstSection: i == 0
                )
    ...
    private static func mapMessagesToRows(messages: [Message], chatType: ChatType, replyMode: ReplyMode, isFirstSection: Bool) -> [MessageRow] {
        ...
        for (i, message) in messages.enumerated() {
            ...
            let positionInUserGroup = PositionInUserGroup.get(
                message: message,
                prev: prev,
                next: next
            )

            let positionInMessagesSection = PositionInMessagesSection.get(
                message: message,
                prev: prev,
                next: next,
            ...
            let commentsPosition = replyMode == .answer ? CommentsPosition.get(message: message, allMessages: messages) : nil
...
public extension ChatView where MessageContent == EmptyView, InputViewContent == EmptyView {
    init(messages: [Message],
         ...
         replyMode: ReplyMode = .quote,
         didSendMessage: @escaping (DraftMessage) -> Void,
         messageMenuAction: @escaping MessageMenuActionClosure) {
        self.type = chatType
        self.animationsEnabled = animationsEnabled
        self.didSendMessage = didSendMessage
        self.sections = ChatView.mapMessages(messages, chatType: chatType, replyMode: replyMode)
